...
export default async function handler(
  ...
  if (req.method !== 'GET') {
    return res.status(HTTP_STATUS.METHOD_NOT_ALLOWED).json({
      ...
      path: req.url || `/api/reports/research/${req.query.nickname}`
    });
  }
  ...
  try {
    ...
    if (!nickname || typeof nickname !== 'string') {
      return res.status(HTTP_STATUS.BAD_REQUEST).json({
        ...
        timestamp: new Date().toISOString(),
        path: req.url || `/api/reports/research/${nickname}`
      });
    }

    const report = await DataLoader.loadResearchReport(nickname);
    ...
    console.error(`API Error in /api/reports/research/${req.query.nickname}:`, error);
